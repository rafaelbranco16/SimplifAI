...
class MedicalController:
    ...
    async def create_clinical_diary(self, nif:str, medical_consultation_text:str):
        try:
            ...
            clinical_diary = await self.service.create_clinical_diary(entry_note, MedicalConsultationText(medical_consultation_text))
    ...
    @param medical_consultation_text - The audio of the medical consultation converted into text
    '''
    async def create_clinical_history(self, nif:str, medical_consultation_text:str):
        try:
            entry_note:EntryNote = await self.entry_note_service.find_entry_note_by_nif(nif)
            ai_text:MedicalConsultationText = await self.service.ai_summary(medical_consultation_text, entry_note)
            clinical_history = await self.service.create_clinical_history(entry_note, ai_text)
            saved_history = await self.service.save_clinical_history(clinical_history)

            return {"message": saved_history}
        except ModuleNotFoundError as not_found:
            Logger.print_error(str(not_found))
            return JSONResponse(
                status_code=404,
